...
export function SubscriptionSettings() {
  ...
  return (
    <div className="px-3 py-2">
      ...
      {paidActive ? (
        <div className="mb-3 text-[16px] font-bold text-accent-800">
        ...
        <div className="mb-3 text-[16px] font-bold text-neutral-600">{t("subs.trialExpired")}</div>
      )}

      <div className="mb-3 rounded bg-accent-100 px-3 py-2 text-[16px] leading-relaxed font-bold text-accent-800">
        {t("subs.valueHook")}
      </div>
            ...
            >
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="text-[16px] font-extrabold">{t(`plan.${plan.cycle}`)}</span>
                  {plan.cycle === "yearly" && (
                    <span className="rounded-full bg-accent px-1.5 py-0.5 text-[16px] font-extrabold tracking-wide text-white uppercase">
                      {t("subs.bestValue")}
                    </span>
                  )}
                </div>
                {plan.hook ? (
                  <div className="text-[16px] font-bold text-accent">{t(`plan_hook.${plan.cycle}`)}</div>
                ) : savings != null ? (
                  <div className="text-[16px] font-bold text-accent">{t("subs.savePct", { pct: savings })}</div>
                ) : null}
              </div>
              <div className="text-right">
                <div className="text-[17px] font-extrabold tabular-nums">{formatVnd(plan.priceVnd)}</div>
                <div className="text-[16px] font-bold text-neutral-500 tabular-nums">{formatUsd(plan.priceUsd)}</div>
              </div>
            </button>
          );
        })}
      ...
      {selectedPlan && (
        <div className="mb-3">
          <button
            type="button"
            className="btn btn-primary btn-block px-4 py-2.5 text-[16px]"
            onClick={checkout}
          ...
          {status === "error" && (
            <div className="mt-1.5 text-[16px] text-red-600">{t("subs.payosError")}</div>
          )}

          <div className="my-2.5 flex items-center gap-2 text-[16px] font-bold uppercase tracking-wide text-neutral-400">
          ...
          <button
            type="button"
            className="btn btn-block px-4 py-2.5 text-[16px]"
          ...
          {paypalStatus === "error" && (
            <div className="mt-1.5 text-[16px] text-red-600">{t("subs.paypalError")}</div>
          )}
        </div>
      )}

      {!isUnlocked && !selectedPlan && (
        <p className="text-[16px] text-neutral-600">{t("subs.footer")}</p>
      )}
    </div>
  );
}
