...
export function ConversationFeedback({ messages, feedback, onReset, share }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        {messages.map((m, i) => {
          ...
          return (
            <div key={i} className="flex flex-col" style={{ alignItems: isUser ? "flex-end" : "flex-start" }}>
              <div
                className="max-w-[92%] rounded p-2.5 text-[16px] leading-relaxed"
              ...
              {turn && ((turn.corrections && turn.corrections.length > 0) || comment || turn.betterExample) && (
                <div
                  className="mt-1 max-w-[92%] rounded border-l-2 p-2 text-[16px] leading-relaxed"
                ...
                >
                  ...
                  {turn.corrections?.map((c, j) => (
                    <p key={j} className="flex flex-wrap items-center gap-1.5">
                      ...
                      <CopyButton text={c.correct} className="rounded-full border px-2 py-0.5 text-[16px] font-bold" style={{ borderColor: "var(--color-accent-800)", color: "var(--color-accent-800)" }} />
                    </p>
                  ))}
                  ...
                  {turn.betterExample && (
                    <p className="mt-1 flex flex-wrap items-center gap-1.5">
                      ...
                      <CopyButton text={turn.betterExample} className="rounded-full border px-2 py-0.5 text-[16px] font-bold" style={{ borderColor: "var(--color-accent-800)", color: "var(--color-accent-800)" }} />
                    </p>
                  )}
                </div>
              )}
      ...
      <div className="rounded border bg-surface p-4 text-[16px] leading-relaxed" style={{ borderColor: "var(--color-divider)" }}>
        ...
        {((style && style.length > 0) || styleHighlight) && (
          <div className="mb-3">
            ...
            {styleHighlight && (
              <p className="mt-1 text-[16px] font-bold" style={{ color: "var(--color-accent)" }}>✅ {styleHighlight}</p>
            )}
          </div>
        )}
          ...
          <div className="mb-3">
            ...
            <div className="flex flex-col gap-1.5">
              {suggestions.map((sug, i) => (
                <div key={i}>
                  <p className="text-[16px] font-bold">{sug.category}</p>
                  <ul className="list-disc pl-4 text-[16px]">
        ...
        {progress && progress.length > 0 && (
          <div className="mb-1">
            ...
            <ul className="list-disc pl-4 text-[16px] italic">
        ...
        {typeof feedback.xpEarned === "number" && (
          <p className="mt-2 text-[16px] font-extrabold text-accent">+{feedback.xpEarned} XP</p>
        )}
        {onReset && <button className="btn btn-ghost mt-3 text-[16px]" onClick={onReset}>{t.tryAgain}</button>}
      </div>
    </div>
  );
}
