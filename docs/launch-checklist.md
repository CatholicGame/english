...
- [x] **Thay "Free/Pro" bằng mô hình trial + gói theo thời hạn** (2026-08-15) — mọi tài khoản mở khoá toàn bộ 7 ngày đầu (`trialStartedAt`, stamp server-side), sau đó khoá lại bình thường cho tới khi mua thêm thời hạn: 1 tháng 90k / 3 tháng 240k / 6 tháng 420k / 12 tháng 720k (`PRICING_PLANS`, tăng giá 2026-08-19 — xem bảng chiết khấu trong [subscription-interim-system.md](./subscription-interim-system.md)).
...
- [ ] Xác nhận lại giá gói 12 tháng (720k là suy luận toán học tiếp nối đường cong chiết khấu 11%/22%, chưa qua kiểm chứng thị trường) — xem [subscription-interim-system.md](./subscription-interim-system.md).
...
- [x] **Quota lượt gọi AI/ngày** (2026-08-17, tăng lên 100 ngày 2026-08-18) — `/api/ai` giờ yêu cầu session (trước đây không check gì) và giới hạn `AI_DAILY_CALL_LIMIT = 100` lượt/ngày/tài khoản (`subscription-store.ts`), đếm nguyên tử qua Firestore transaction (`checkAndIncrementAiUsage` trong `subscription-db.ts`, field `aiCallsToday`/`aiCallsDate` trong chính doc `subscriptions/{email}`), trả `429` kèm message khi vượt. Cùng 1 ngưỡng cho mọi gói (1/3/6/12 tháng chỉ khác thời hạn, không khác mức truy cập). Ban đầu đặt 40 dựa trên ước tính bi quan 30đ/lượt (giờ peak); số liệu thật từ DeepSeek usage dashboard (2026-08-18: $0.13/246 requests/539K tokens) cho thấy chi phí thật ≈ 13,7đ/lượt — thấp hơn nhiều — nên nâng lên 100/ngày (≈1.370đ/ngày, vẫn dưới doanh thu/ngày gói rẻ nhất 1.667đ/ngày) để đủ chỗ cho các phiên hội thoại dài (mỗi lượt nhắn trong Converse/Discussion tính 1 call riêng, 1 phiên có thể tốn 15-30 lượt).
