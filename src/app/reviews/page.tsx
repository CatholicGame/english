...
export default function ReviewsPage() {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-[480px] flex-col bg-bg lg:max-w-[1080px] lg:border-x-2 lg:border-[color:var(--color-divider)]">
      ...
      {error && <p className="px-4 py-6 text-[16px] text-accent-700">{t("reviews.loadError")}</p>}

      {!error && reviews === null && <p className="px-4 py-6 text-[16px] text-neutral-500">{t("reviews.loading")}</p>}
        ...
        <>
          <div className="divider-b flex gap-6 px-4 py-5">
            <div className="flex-none text-center">
              ...
              <div className="mt-1 text-[16px] text-neutral-600">{t("reviews.count", { n: stats.count })}</div>
            </div>
            <div className="flex flex-1 flex-col justify-center gap-1">
              {RATING_ROWS.map((n) => {
                ...
                return (
                  <div key={n} className="flex items-center gap-2 text-[16px]">
          ...
          <div className="flex-1 px-4 py-4">
            ...
            {reviews.length === 0 && <p className="text-[16px] text-neutral-500">{t("reviews.empty")}</p>}
            <div className="flex flex-col gap-3">
              {reviews.map((r) => (
                <div key={r.id} className="border-b pb-3" style={{ borderColor: "var(--color-divider)" }}>
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[16px] font-extrabold">{r.name || t("reviews.anonymous")}</span>
                  ...
                  {r.comment && <p className="mt-1.5 whitespace-pre-wrap text-[16px] leading-relaxed">{r.comment}</p>}
                  {r.reply && (
                    <div className="mt-2 border-l-2 pl-2.5" style={{ borderColor: "var(--color-accent)" }}>
                      ...
                      <p className="mt-0.5 whitespace-pre-wrap text-[16px] leading-relaxed">{r.reply.message}</p>
                    </div>
                  )}
                </div>
              ))}
