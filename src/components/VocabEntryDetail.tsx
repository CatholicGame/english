  ...
  return (
    <>
      ...
      <div className="mb-4 pr-6">
        ...
        {data?.ipa && <span className="text-[13px] text-neutral-600">{data.ipa}</span>}
      </div>
        ...
        <ol className="flex flex-col gap-3">
          {senses.map((s, i) => (
            <li key={i} className="flex gap-2">
              <span className="mt-[1px] flex-none text-[13px] font-extrabold text-neutral-500">{i + 1}.</span>
              <div className="min-w-0">
                <div className="flex flex-wrap items-baseline gap-x-2">
                  {s.pos && <span className="label-xs text-accent">{s.pos}</span>}
                  {s.vi && <span className="text-[14px] font-bold">{s.vi}</span>}
                </div>
                {s.en && <p className="mt-0.5 text-[13px] leading-relaxed text-neutral-700">{s.en}</p>}
                {s.example && (
                  <div className="mt-1.5 pl-2.5" style={{ borderLeft: "2px solid var(--color-divider)" }}>
                    <p className="text-[13px] italic leading-relaxed">&quot;{s.example.en}&quot;</p>
                    {s.example.vi && <p className="mt-0.5 text-[12px] text-neutral-600">{s.example.vi}</p>}
                  </div>
                )}
              </div>
            </li>
        ...
        <div className="mt-4">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {data.synonyms.map((s, i) => (
              <span key={i} className="border px-2 py-0.5 text-[12px]" style={{ borderColor: "var(--color-divider)" }}>{s}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {/* Memory tip */}
      {data?.memoryTip && (
        <div className="mt-4 bg-accent-100 p-3 text-[13px] leading-relaxed text-accent-700">
