...
import { readDriveProgress, writeDriveProgress, type DriveProgressData } from "@/lib/google-drive";
...
export async function GET(request: NextRequest) {
  ...
  const { data, files } = await readDriveProgress(auth.accessToken, auth.session, storageKey);
...
export async function PUT(request: NextRequest) {
  ...
  const body: DriveProgressData = await request.json();

  const auth = await getValidAccessToken();
  if (!auth) return NextResponse.json({ reason: "reauth_required" }, { status: 401 });

  const { files } = await writeDriveProgress(auth.accessToken, auth.session, storageKey, body);
  if (filesChanged(auth.session.files, files)) {
    await writeSession({ ...auth.session, files });
  }
