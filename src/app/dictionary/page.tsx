...
function VocabTab({ query }: { query: string }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        type="button"
        className="btn btn-primary mb-1 w-full py-2.5 text-[16px]"
        ...
        <div key={key} className="border border-transparent bg-surface p-3">
          ...
          >
            <span className="flex flex-wrap items-baseline gap-x-2">
              <span className="text-[19px] font-extrabold">{entry.word}</span>
              <CategoryBadge category={entry.category} />
              {entry.senses?.[0]?.pos && <span className="text-[16px] italic text-neutral-600">{entry.senses[0].pos}</span>}
            </span>
            <span className="label-xs whitespace-nowrap text-accent">{openKey === key ? "Thu gọn" : "Xem"}</span>
          </button>
          {entry.senses?.[0]?.vi && openKey !== key && (
            <p className="mt-1 truncate text-[16px] text-neutral-700">{entry.senses[0].vi}</p>
          )}
          {openKey === key && (
            <div className="mt-3 border-t pt-3" style={{ borderColor: "var(--color-divider)" }}>
              ...
              <button
                type="button"
                className="btn btn-ghost mt-2 text-[16px] text-red-600"
        ...
        <div key={key} className="border border-transparent bg-surface p-3">
          ...
          >
            <span className="min-w-0">
              <span className="flex flex-wrap items-baseline gap-x-2">
                <span className="text-[16px] font-extrabold text-accent-800">📐 {entry.category}</span>
                {entry.discussed ? (
              ...
              <p className="mt-1 truncate text-[16px] text-neutral-700">{entry.text}</p>
            </span>
...
function TranslationsTab({ query }: { query: string }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {list.map(([key, entry]) => (
        <div key={key} className="border border-transparent bg-surface p-3">
          <p className="text-[17px] leading-relaxed">{entry.text}</p>
          <p className="mt-1.5 text-[16px] leading-relaxed text-neutral-700">{entry.translation}</p>
          <button
            type="button"
            className="btn btn-ghost mt-2 text-[16px] text-red-600"
            onClick={() => deleteEntry(key)}
