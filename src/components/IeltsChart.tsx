...
function Legend({ series }: { series: ChartPanel["series"] }) {
  ...
  return (
    <div className="mb-2 flex flex-wrap gap-x-3 gap-y-1">
      {series.map((s, i) => (
        <span key={s.key} className="flex items-center gap-1.5 text-[11px] text-neutral-600">
          <span
    ...
    <div>
      ...
      <div className="flex flex-col gap-2.5">
        {panel.groups.map((g) => {
          ...
          return (
            <div key={g.label}>
              <div className="mb-1 text-[11px] font-bold text-neutral-700">{g.label}</div>
              ...
              {panel.series.map((s, i) => {
                ...
                return (
                  <div key={s.key} className="flex items-center gap-1.5" title={`${s.label}: ${v}%`}>
                    ...
                    <span className="w-9 flex-none text-right text-[10px] tabular-nums text-neutral-600">{v}%</span>
                  </div>
                );
              })}
            </div>
