...
import { readDriveDictionary, writeDriveDictionary } from "@/lib/google-drive";
...
export async function GET() {
  ...
  const { data, files } = await readDriveDictionary(auth.accessToken, auth.session);
...
export async function PUT(request: NextRequest) {
  const body: DictionaryData = await request.json();

  const auth = await getValidAccessToken();
  if (!auth) return NextResponse.json({ reason: "reauth_required" }, { status: 401 });

  const { files } = await writeDriveDictionary(auth.accessToken, auth.session, body);
  if (filesChanged(auth.session.files, files)) {
    await writeSession({ ...auth.session, files });
  }
