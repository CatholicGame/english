...
/** The unit list of one Grammar in Use book, shared by every book's module
 * route (see src/data/grammar-books.ts). */
export function GrammarUnitsList({ moduleSlug }: { moduleSlug: string }) {
  const { title, meta: UNITS_META, sections: GRAMMAR_SECTIONS } = getGrammarBook(moduleSlug);
  ...
  useEffect(() => setResume(latestUnfinished((slug) => UNITS_META.some((u) => u.slug === slug))), [UNITS_META]);
  ...
  // which unit number that is. The book itself expects this: "The units
    ...
    <div className="flex-1 lg:flex lg:flex-row lg:items-stretch lg:gap-8 lg:px-4 lg:py-6">
      ...
      <div className="lg:flex-1">
        {/* Unit left half-finished: the session is kept now, so the way back
            into it should be one tap and not a hunt through the whole list. */}
        {resumeUnit && (
          <div className="px-4 pt-4 lg:px-0">
            <Link
              href={`/modules/${moduleSlug}/${resumeUnit.slug}`}
              ...
              <div className="lg:grid lg:grid-cols-2 lg:gap-x-4">
                {section.units.map((u) => {
                  ...
                  return u.available ? (
                    <Link
                      key={u.slug}
                      href={`/modules/${moduleSlug}/${u.slug}`}
