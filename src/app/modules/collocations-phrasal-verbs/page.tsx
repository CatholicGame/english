...
export default function TodayPage() {
  ...
  return (
    <div>
        ...
        <div className="flex h-16 items-end justify-between gap-1">
          {week.map((d, i) => (
            <div key={i} className="flex flex-1 flex-col items-stretch gap-1.5">
              ...
              <span className="text-center text-[16px] tracking-wider text-neutral-600">{d.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="divider-b px-4 py-4">
        <button
          className="btn btn-primary btn-block px-4 py-3 text-[18px]"
        ...
        <div className="mt-2 text-[16px] text-neutral-600">{due} phrases still to master</div>

        {hasMistakes && (
          <div className="mt-3">
            <button
              className="btn btn-accent btn-block px-4 py-2.5 text-[16px]"
              onClick={() => {
      ...
      <div className="divider-t grid grid-cols-2 gap-[2px] bg-[color:var(--color-divider)] lg:grid-cols-3">
        {practiceModes.map((m) => (
          ...
          >
            <span className="block text-[18px] font-extrabold">{m.title}</span>
            <span className="mt-1 block text-[16px] text-neutral-600">{m.sub}</span>
          </button>
        ))}
      </div>
    </div>
