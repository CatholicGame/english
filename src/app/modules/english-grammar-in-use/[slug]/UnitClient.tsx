      ...
      {alt && (
        <>
          <button
            ...
            aria-label={altTitle}
            title={altTitle}
            className="mx-1 inline-block -translate-y-px rounded px-1 py-px align-middle text-[16px] font-extrabold"
            style={
              shown
          ...
          {shown && <span className="mt-0.5 block text-[16px] text-neutral-500 italic">{renderRich(alt)}</span>}
        </>
      )}
    </span>
  );
    ...
    return (
      <div className="border-l-2 pl-3" style={{ borderColor: "var(--color-accent-300)" }}>
        {table.rows.map((row, i) => (
          <div key={i} className={`flex flex-wrap gap-x-3 text-[16px] leading-relaxed ${i > 0 ? "mt-1" : ""}`}>
...
function RuleSituationView({ part }: { part: RuleSituation }) {
  ...
  return (
    <div className="rounded-lg border p-3" style={{ borderColor: "var(--color-divider)", background: "var(--color-bg)" }}>
      <Explanation en={part.text} vi={part.vi} className="block text-[16px] leading-relaxed text-neutral-700" />
      {part.quotes?.map((q, i) => (
        <div key={i} className="mt-2.5">
          {q.speaker && <span className="label-xs mb-0.5 block text-neutral-500">{q.speaker}</span>}
          <div
            className="relative inline-block rounded-xl border px-3 py-2 text-[16px] leading-relaxed"
  ...
  return (
    <div>
      ...
      <ul className="flex flex-col gap-1.5">
        {part.items.map((ex, j) => (
          <li key={j} className="border-l-2 border-neutral-300 pl-2.5 text-[16px]">
...
function RulePartView({ part }: { part: RulePart }) {
  switch (part.kind) {
    ...
    case "text":
      return <Explanation en={part.text} vi={part.vi} className="block text-[16px] leading-relaxed text-neutral-700" />;
    case "table":
      ...
      return (
        <div className="flex flex-wrap gap-1.5">
          {part.words.map((w, k) => (
            <span
              key={k}
              className="rounded-full border px-2.5 py-1 text-[16px]"
    ...
    <div className="overflow-hidden rounded-lg bg-surface">
      {(b.label || b.heading) && (
        <div className="flex items-center gap-2 px-3 py-2" style={{ background: "var(--color-accent)" }}>
          {b.label && (
            <span className="flex h-5 w-5 flex-none items-center justify-center rounded-full bg-white/25 text-[16px] font-extrabold text-white">
              {b.label}
            </span>
          )}
          {b.heading && (
            <span className="text-[17px] leading-snug font-extrabold text-white">
      ...
      <div className="flex flex-col gap-2.5 p-3">
        {b.intro && (
          <Explanation en={b.intro} vi={b.introVi} className="block text-[16px] font-bold text-neutral-500" />
        )}
        {b.parts.map((part, i) => (
    ...
    <div className="mb-3 flex items-center justify-end gap-1">
      {[false, true].map((value) => (
        <button
          ...
          className="rounded-full border px-2.5 py-1 text-[16px] font-bold"
          style={
            original === value
...
function ExerciseHeader({ step, onPickWord }: { step: ChromeStep; onPickWord?: (word: string) => void }) {
  ...
  return (
    <div className="mb-4">
      <h2 className="text-[18px] leading-snug font-extrabold">{loc(step.title, step.titleEn, lang)}</h2>
      <p className="mt-1 text-[16px] leading-relaxed text-neutral-600">
        {renderRich(loc(step.instructions, step.instructionsEn, lang))}
      </p>
      {passage && (
        <div
          className="mt-3 rounded-md border-l-[3px] bg-surface px-3 py-2.5 text-[16px] leading-relaxed"
      ...
      {step.wordBank && step.wordBank.length > 0 && (
        ...
        >
            ...
            {step.wordBank.map((w) =>
              onPickWord ? (
                <button
                  ...
                  className="rounded-full border bg-surface px-2.5 py-1 text-[16px] font-bold"
                  style={{ borderColor: "var(--color-accent-300)" }}
                >
                  {w}
                </button>
          ...
          {onPickWord && <p className="mt-1.5 text-[16px] text-neutral-500">{t("grammar.wordBankHint")}</p>}
        </div>
      )}
    </div>
  );
...
function ItemRow({ label, tone = "idle", children }: { label: string; tone?: ItemTone; children: React.ReactNode }) {
  return (
    <div className="flex gap-2.5 py-1.5">
      <span
        className="mt-[3px] flex h-[22px] min-w-[22px] flex-none items-center justify-center rounded-full px-1 text-[16px] font-extrabold tabular-nums"
      ...
      {parts.map((part, i) => (
        <span key={i}>
          {i > 0 &&
            (renderBlank?.(i - 1) ?? (
              <span
                className="mx-1 inline-block w-[72px] border-b-2 border-dashed text-center align-middle text-[16px] leading-[14px] font-extrabold text-accent"
                style={{ borderColor: "var(--color-accent-300)" }}
              >
                {gaps > 1 ? i : ""}
              </span>
...
function ContextGroup({ of, children }: { of: Contextual; children: React.ReactNode }) {
  ...
  return (
    <div className="mb-3 overflow-hidden rounded-lg border" style={{ borderColor: "var(--color-divider)" }}>
      <p className="bg-surface px-3 py-2 text-[16px] leading-relaxed text-neutral-700">
    ...
    return (
      <>
        {total != null && done != null && (
          <div className="mb-1.5 text-center text-[16px] tabular-nums text-neutral-500">
  ...
  return (
    <>
      {correct != null && total != null && (
        <div className="mb-1.5 flex items-center justify-center gap-1.5 text-[16px]">
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                      {it.options.map((o) => {
                        ...
                        return (
                          <button
                            key={o}
                            className="rounded-full border px-3 py-1.5 text-[16px] font-bold"
                            style={{
        ...
        <div className="flex flex-col gap-1.5">
          {step.left.map((text, i) => (
            <button
              ...
              style={styleFor("l", i)}
              className="rounded-md border px-2.5 py-2 text-left text-[16px] leading-snug"
              onClick={() => pick("l", i)}
            >
                  ...
                  {!checked &&
                    row.map((_, j) => (
                      <div key={j} className="mt-1 flex items-center gap-1.5">
                        {row.length > 1 && <span className="w-3 flex-none text-[16px] font-extrabold text-accent">{j + 1}</span>}
                        <input
                ...
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                  {[
                    ...
                    return (
                      <button
                        key={opt.label}
                        className="rounded-full border px-3 py-1 text-[16px] font-bold"
                        style={{
  ...
  return (
    <div className="flex flex-1 flex-col p-4">
      ...
      {result != null && xpEarned != null && (
        <div className="mt-2 text-[16px] font-extrabold text-accent">+{xpEarned} XP</div>
      )}
      {inlineAction}
    </div>
  );
            ...
            <div className="fixed inset-0 z-[60] bg-bg">
              <div className="mx-auto flex h-full max-w-[480px] flex-col lg:max-w-[min(90vw,2400px)]">
                <div className="divider-b flex items-center justify-between px-4 py-3">
                  <span className="text-[19px] font-extrabold">{t("grammar.stepListTitle")}</span>
