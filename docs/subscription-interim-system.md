...
(list Cambridge/Listen/Verbs, trang Write) → mở PurchaseModal
        │
        ▼
Người dùng chọn 1 gói trong popup → bấm "Thanh toán qua PayOS"
        │
        ▼
...
| File | Vai trò |
|---|---|
| `src/lib/subscription-store.ts` | `SubscriptionData`, `PRICING_PLANS`, `isUnlocked`/`isTrialActive`/`isPaidActive`/`trialDaysLeft`, `withPaidExtended`, `mergeSubscription` — không đổi so với trước |
...
| `src/lib/subscription-db.ts` | Firestore CRUD: `getSubscription`/`setSubscription` (collection `subscriptions`), `createPendingOrder`/`getOrder`/`markOrderPaidOnce` (collection `payos_orders`) |
...
| `src/app/page.tsx` | `usePayosReturn()` — xử lý `?payos=success`/`?payos=cancel` khi PayOS redirect về, poll `refetch()` tới khi thấy `paidUntil` mới hoặc timeout |
| `src/components/SubscriptionSettings.tsx` | Trạng thái + bảng giá + nút "Thanh toán qua PayOS" (redirect sang `checkoutUrl`) — dùng chung ở Settings VÀ trong `PurchaseModal` |
...
6. **`NEXT_PUBLIC_APP_URL` cần trỏ đúng domain thật** (xem bảng env vars) — nếu thiếu, `returnUrl`/`cancelUrl` sẽ dùng fallback domain cũ và PayOS sẽ redirect sai chỗ sau khi thanh toán.
