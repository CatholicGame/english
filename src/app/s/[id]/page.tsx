  ...
  if (!data) {
    return (
      <main className="flex flex-1 flex-col items-center justify-center gap-2 p-6 text-center">
        <p className="text-[17px] font-extrabold">Link không hợp lệ hoặc đã hỏng</p>
        <p className="text-[16px] text-neutral-500">Nội dung được chia sẻ không thể đọc được.</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-lg flex-1 flex-col gap-3 p-3">
      <div className="border-b pb-2" style={{ borderColor: "var(--color-divider)" }}>
        ...
        <h1 className="text-[19px] font-extrabold">{data.itemLabel}</h1>
      </div>
      {data.feedback ? (
      ...
      <p className="mt-2 border-t pt-2 text-center text-[16px] text-neutral-400" style={{ borderColor: "var(--color-divider)" }}>
